...
// ─── SKU map ──────────────────────────────────────────────────────────────────
// Prefijo del SKU → id de categoría
const SKU_MAP = {
  'RN': 'remera-negra',
  'RB': 'remera-blanca',
  'BN': 'buzo-negro',
...
function saveCategorias(cats) {
  ...
  // Actualizar SKU_MAP dinámicamente
...
app.post('/api/categorias', auth, (req, res) => {
  ...
  if (cats.find(c => c.id === id)) return res.status(400).json({ error: 'Ya existe esa categoría' });
  if (cats.find(c => c.skuPrefix === prefix)) return res.status(400).json({ error: 'Ese prefijo SKU ya está en uso' });
  cats.push({ id, nombre, emoji: emoji || '📦', skuPrefix: prefix });
  saveCategorias(cats);
  addLog({ tipo: 'config', mensaje: `Nueva categoría: ${nombre} (SKU prefix: ${prefix})` });
...
app.delete('/api/categorias/:id', auth, (req, res) => {
  ...
  saveCategorias(cats);
  addLog({ tipo: 'config', mensaje: `Categoría eliminada: ${req.params.id}` });
  res.json({ ok: true, categorias: cats });
});
...
app.put('/api/stock', auth, (req, res) => {
  ...
  if (!SIZES.includes(size) || typeof quantity !== 'number') return res.status(400).json({ error: 'Datos inválidos' });
...
app.post('/webhook', async (req, res) => {
  if (WH_SECRET && req.headers['x-linkedstore-token'] !== WH_SECRET) return res.status(401).json({ error: 'Token inválido' });
